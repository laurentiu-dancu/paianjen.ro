defmodule PaianjenWeb.ListingLive.Show do
  ...
  def mount(%{"id" => id}, _session, socket) do
    group = MockData.get_group(id)

    {:ok,
     assign(socket,
       page_title: group && group.zone || group && group.district || "Detalii",
       group: group,
       all_groups: MockData.all_groups()
     )}
  end
  ...
  def render(assigns) do
    ...
    <div class="min-h-screen bg-gray-50">
      ...
      <main :if={@group} class="max-w-7xl mx-auto px-4 py-8">
          ...
          <div class="flex items-start justify-between mb-6">
            ...
            <div class="flex items-center gap-3">
              <div class="text-right">
                ...
                <%
                  health_pct = (@group.active_listings / @group.total_listings) * 100
        ...
        <%!-- Similar Groups --%>
        <%
          similar_groups = if @group do
            price_range = @group.average_price * 0.15
            @all_groups
            |> Enum.filter(fn g ->
              g.id != @group.id &&
                ((abs(g.average_price - @group.average_price) < price_range && abs(g.surface_area - @group.surface_area) <= 10) ||
                 (g.district == @group.district && abs(g.surface_area - @group.surface_area) <= 10))
            end)
            |> Enum.take(3)
          else
            []
